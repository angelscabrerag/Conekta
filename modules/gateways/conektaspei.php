...
function conektaspei_link($params) {
	...
	{
	  error_log('Error Conekta SPEI: Código: ' . $e->getLine() . '. ' . $e->getMessage() . '. URL: ' . $_SERVER[HTTP_HOST] . $_SERVER['REQUEST_URI'], 1, 'contacto@multieconomico.com.mx');
	  if($e->getLine() == 35)
	  {
	  	$code =  "
	  		<div class='alert alert-danger'>
	  			Error al intentar generar la orden de pago mediante SPEI debido a que la fecha limite de pago vencio. Por favor, intenta con otra forma de pago.
	  		</div>
	  	";
	  } else {
	  	$code =  "
	  		<div class='alert alert-danger'>
	  			Error al intentar generar la orden de pago mediante SPEI. Por favor, intenta con otra forma de pago.
	  		</div>
	  	";
	  }
	  $ticket = 0;
